...
function signToken(user: { id: number; username: string; role: string; linked_driver_id?: number | null }) {
  return jwt.sign(
    { userId: user.id, username: user.username, role: user.role, linkedDriverId: user.linked_driver_id ?? null },
    JWT_SECRET,
    { expiresIn: '7d' }
  );
}
...
router.get('/me', authMiddleware, async (req, res) => {
  try {
    const user = await getOne('SELECT id, username, role, display_name, linked_driver_id FROM users WHERE id = $1', [req.user!.id]);
...
router.get('/users', authMiddleware, async (req, res) => {
  try {
    ...
    const users = await getAll(`
      SELECT u.id, u.username, u.role, u.display_name, u.is_active, u.created_at, u.linked_driver_id, d.name as driver_name,
...
router.post('/users', authMiddleware, async (req, res) => {
  try {
    if (req.user!.role !== 'owner') return res.status(403).json({ error: 'Owner only' });
    const { username, password, display_name, role, driver_id } = req.body;
    ...
    const result = await getOne(
      'INSERT INTO users (username, password_hash, role, display_name, linked_driver_id) VALUES ($1,$2,$3,$4,$5) RETURNING id, username, role, display_name, linked_driver_id, created_at',
      [username, hash, role, display_name, linkedDriverId]
    );
...
router.put('/users/:id', authMiddleware, async (req, res) => {
  try {
    if (req.user!.role !== 'owner') return res.status(403).json({ error: 'Owner only' });
    const { display_name, role, is_active, driver_id } = req.body;
    const row = await getOne(
      'UPDATE users SET display_name=COALESCE($1,display_name), role=COALESCE($2,role), is_active=COALESCE($3,is_active), linked_driver_id=COALESCE($4,linked_driver_id) WHERE id=$5 RETURNING id, username, role, display_name, is_active, linked_driver_id',
      [display_name ?? null, role ?? null, is_active ?? null, driver_id ?? null, req.params.id]
    );
